...
preserve 
	...
	*Looking at triplets where subjec=nature & object=human/manmade. 
	gen nature_scl_human_ocl=1 if nature_scl==1 & (human_ocl==1 | manmade_ocl==1)
	gen nature_scl_nohuman_ocl=1 if nature_scl==1 & human_ocl==0 & manmade_ocl==0 & object!=""
	gen human_scl_nature_ocl=1 if human_scl==1 & nature_ocl==1 
	...
	collapse (sum) n_triplets_socl=obs_tag nature_scl_human_ocl human_scl_nature_ocl nature_scl_or_ocl nature_scl_major_ocl nature_scl_nohuman_ocl, by(motif_id)

	gen d_nature_scl_human_ocl=(nature_scl_human_ocl>0) if nature_scl_human_ocl!=.
	gen d_nature_scl_nohuman_ocl=(nature_scl_nohuman_ocl>0) if nature_scl_nohuman_ocl!=.
...
preserve
	...
	*Where (1) subject is nature (2) evaluation is positive or negative
	gen nature_epos=1 if nature_scl==1 & evaluation>=0 & evaluation!=.
	gen nature_eneg=1 if nature_scl==1 & evaluation<0
	
	*Where (1) subject is nature (2) potency is positive or negative
	gen nature_ppos_o=1 if nature_ocl==1 & potency>=0 & potency!=.
	gen nature_pneg_o=1 if nature_ocl==1 & potency<0

	*Where (1) subject is nature (2) evaluation is positive or negative
	gen nature_epos_o=1 if nature_ocl==1 & evaluation>=0 & evaluation!=.
	gen nature_eneg_o=1 if nature_ocl==1 & evaluation<0
	...
	*keeping only triplets with some sort ofclassification 
	...
	gen d_nature_ppos_o=(nature_ppos_o>0) if nature_ppos_o!=.
	gen d_nature_pneg_o=(nature_pneg_o>0) if nature_pneg_o!=.
	gen d_nature_epos_o=(nature_epos_o>0) if nature_epos_o!=.
	gen d_nature_eneg_o=(nature_eneg_o>0) if nature_eneg_o!=.
...
preserve
	...
	gen nature_scl_super_ocl=1 if nature_scl==1 & (supernatural_ocl==1 | hybrid_ocl==1)
	replace nature_scl_super_ocl=. if nature_scl_human_ocl==1
	
	gen nature_scl_nature_ocl=1 if nature_scl==1 & nature_ocl==1
	replace nature_scl_nature_ocl=. if nature_scl_human_ocl==1
	
	gen nature_scl_miss_ocl=1 if nature_scl==1 & object==""
	...
	*keeping only triplets with some sort ofclassification 
	...
	collapse (sum) n_triplets_act=obs_tag nature_scl_super_ocl nature_scl_human_ocl nature_scl_nature_ocl nature_scl_miss_ocl, by(motif_id)
	
	gen d_nature_scl_super_ocl=(nature_scl_super_ocl>0) if nature_scl_super_ocl!=.
	gen d_nature_scl_human_ocl=(nature_scl_human_ocl>0) if nature_scl_human_ocl!=.
	gen d_nature_scl_nature_ocl=(nature_scl_nature_ocl>0) if nature_scl_nature_ocl!=.
	gen d_nature_scl_miss_ocl=(nature_scl_miss_ocl>0) if nature_scl_miss_ocl!=.

	tempfile Triplets_nature_ocl
...
*
*-------------------------------------------------------------------------------
use "${data}/interim\Berezkin_groups_noEA.dta", clear 

keep if sample_IVS_noEA==1

ren group_Berezkin group_berezkin
rename (a1-n9) motif_id_=

reshape long motif_id_, i(group_berezkin) j(motif_id) string

keep if motif_id_ == 1

ren motif_id_ n_motifs

drop oid-sample_IVS_noEA 

*Merging Folklore measures 
merge m:1 motif_id using `Triplets_scl', keep(1 2 3) gen(merge_triplet_ocl)
...
bys group_berezkin: egen ea_n_motifs=sum(n_motifs)

foreach var in evaluation potency activity evaluation_v2 potency_v2 activity_v2 {
	
	cap drop act_n_motifs
	bys group_berezkin `var': egen act_n_motifs=sum(n_motifs)
	gen w_`var'= (act_n_motifs*`var')/ea_n_motifs
	
}

collapse (sum) n_motifs n_triplets* human* nature* d_human* d_nat* d_super* super* (mean) w_* evaluation potency activity evaluation_v2 potency_v2 activity_v2, by(group_berezkin)
...
gen sh_natsubj_nohumobj_motif_atl=d_nature_scl_nohuman_ocl/n_motifs
...
gen sh_nature_ppos_act_oatl=d_nature_ppos_o/n_motifs
gen sh_nature_pneg_act_oatl=d_nature_pneg_o/n_motifs
gen sh_nature_epos_act_oatl=d_nature_epos_o/n_motifs
gen sh_nature_eneg_act_oatl=d_nature_eneg_o/n_motifs
...
*Nature and different object types
gen sh_nature_scl_super_ocl_atl=d_nature_scl_super_ocl/n_motifs
...
la var sh_natsubj_nohumobj_motif_atl "Share of motifs with at least one triplet with a nature subject and a non-human object"
...
la var sh_natsubj_nohumobj_motif_atl "Sh of motifs w at least one triplet w a nature subject and a non-human object"
...
la var sh_nature_ppos_act_oatl "Sh of motifs w at least one triplet w nature object and positive potency"
la var sh_nature_pneg_act_oatl "Sh of motifs w at least one triplet w nature object and negative potency"
la var sh_nature_epos_act_oatl "Sh of motifs w at least one triplet w nature object and positive evaluation"
la var sh_nature_eneg_act_oatl "Sh of motifs w at least one triplet w nature object and negative evaluation"
...
la var sh_nature_scl_super_ocl_atl "sh of motifs w at least one triplet w nature subject and supernatural object"
la var sh_nature_scl_human_ocl_atl "sh of motifs w at least one triplet w nature subject and human object"
la var sh_nature_scl_nature_ocl_atl "sh of motifs w at least one triplet w nature subject and nature object"
...
save "${data}/interim\Motifs_NO_EA_WESEE.dta", replace
