...
reg index1_all sh_nature_smotif_atleast_nonexcl sh_nature_omotif_atleast_nonexcl
matrix B = e(b)
global b = string(round(B[1,1], .001), "%9.3f")
...
ytitle("") l2title("Pro-environment support index", size(medium))  ///
text(.35 .65 "{&beta} = ${b}", size(medsmall)) ///
controls(sh_nature_omotif_atleast_nonexcl)

gr export "${plots}/binscatter_sh_nature_subject_X.pdf", as(pdf) replace

reg index1_all sh_nature_omotif_atleast_nonexcl sh_nature_smotif_atleast_nonexcl
matrix B = e(b)
global b = string(round(B[1,1], .001), "%9.3f")
...
ytitle("") l2title("Pro-environment support index", size(medium))  ///
text(.3 .43 "{&beta} = ${b}", size(medsmall)) ///
controls(sh_nature_smotif_atleast_nonexcl)

gr export "${plots}/binscatter_sh_nature_object_X.pdf", as(pdf) replace
