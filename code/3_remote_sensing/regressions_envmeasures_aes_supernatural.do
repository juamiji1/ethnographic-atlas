...
AUTHOR: JMJR
TOPIC: Regressions with supernatural variables
DATE:
...
cap drop sh_nat_smotif_atleast sh_nat_omotif_atleast sh_super_smotif_atleast sh_super_omotif_atleast
gen sh_nat_smotif_atleast=sh_nature_smotif_atleast_nonexcl
gen sh_nat_omotif_atleast=sh_nature_omotif_atleast_nonexcl
gen sh_super_smotif_atleast=sh_supernat_smotif_atl_nonexcl
gen sh_super_omotif_atleast=sh_supernat_omotif_atl_nonexcl

gen sh_hum_smotif_atleast=sh_human_smotif_atleast_nonexcl
gen sh_hum_omotif_atleast=sh_human_omotif_atleast_nonexcl
...
la var sh_super_smotif_atleast `" "Share of motifs with a" "supernatural subject" "'
la var sh_super_omotif_atleast `" "Share of motifs with a" "supernatural object" "'
la var sh_hum_smotif_atleast `" "Share of motifs with a" "human subject" "'
la var sh_hum_omotif_atleast `" "Share of motifs with a" "human object" "'

*-------------------------------------------------------------------------------
* Results 
*
*-------------------------------------------------------------------------------
gl X1_int "sh_nat_smotif_atleast sh_nat_omotif_atleast sh_hum_smotif_atleast sh_hum_omotif_atleast"
gl X2_int "sh_nat_smotif_atleast sh_nat_omotif_atleast sh_super_smotif_atleast sh_super_omotif_atleast"
gl X3_int "sh_nat_smotif_atleast sh_nat_omotif_atleast sh_hum_smotif_atleast sh_hum_omotif_atleast sh_super_smotif_atleast sh_super_omotif_atleast"
...
** Column 3 - X1_int + HII + Climatic zones
drop missing_values
egen missing_values = rowmiss(bii sh_treecover changewater ${X1_int} hii)
...
** Column 4 - X2_int (Nature + Supernatural)
drop missing_values
...
** Column 6 - X2_int + HII + Climatic zones
drop missing_values
egen missing_values = rowmiss(bii sh_treecover changewater ${X2_int} hii)
...
** Column 7 - X3_int (Nature + Human + Supernatural)
drop missing_values
egen missing_values = rowmiss(bii sh_treecover changewater ${X3_int})

eststo aes6: avg_effect bii sh_treecover changewater if missing_values==0, x(${X0} ${X3_int}) effectvar(${X3_int}) controltest(missing_values==0) cl(eafolk_id)
gl n6 = "`e(N)'"
...
** Column 8 - X3_int + HII
drop missing_values
egen missing_values = rowmiss(bii sh_treecover changewater ${X3_int} hii)

eststo aes7: avg_effect bii sh_treecover changewater if missing_values==0, x(${X1} ${X3_int}) effectvar(${X3_int}) controltest(missing_values==0) cl(eafolk_id)
gl n7 = "`e(N)'"
...
** Column 9 - X3_int + HII + Climatic zones
drop missing_values
egen missing_values = rowmiss(bii sh_treecover changewater ${X3_int} hii)

eststo aes8: avg_effect bii sh_treecover changewater if missing_values==0, x(${X2} ${X3_int}) effectvar(${X3_int}) controltest(missing_values==0) cl(eafolk_id)
gl n8 = "`e(N)'"
...
esttab aes0 aes1 aes2 aes3 aes4 aes5 aes6 aes7 aes8 using "${tables}/Table_folklore_z_env_supernatural.tex", keep(ae_sh_nat_smotif_atleast ae_sh_nat_omotif_atleast ae_sh_super_smotif_atleast ae_sh_super_omotif_atleast ae_sh_hum_smotif_atleast ae_sh_hum_omotif_atleast) ///
	coeflabels( ///
    ...
    ae_sh_super_smotif_atleast "\multirow{2}{*}{\shortstack{Share of motifs with at least one supernatural\\ \hspace{1em}subject in a triplet}}" ///
    ae_sh_super_omotif_atleast "\multirow{2}{*}{\shortstack{Share of motifs with at least one supernatural\\ \hspace{1em}object in a triplet}}" ///
    ae_sh_hum_smotif_atleast "\multirow{2}{*}{\shortstack{Share of motifs with at least one human\\ \hspace{1em}subject in a triplet}}" ///
    ae_sh_hum_omotif_atleast "\multirow{2}{*}{\shortstack{Share of motifs with at least one human\\ \hspace{1em}object in a triplet}}") ///
    se nocons star(* 0.10 ** 0.05 *** 0.01) ///
    label nolines fragment nomtitle nonumbers noobs nodep collabels(none) ///
    booktabs b(3) replace ///
	prehead(`"\begin{tabular}[t]{l*{9}{c}}"' ///
			`"\toprule"' ///
			`" & \multicolumn{9}{c}{Environmental Measures (AES)} \\"' ///
			`"\cmidrule(lr){2-10}"' ///
			`" & \multicolumn{3}{c}{Nature + Human} & \multicolumn{3}{c}{Nature + Supernatural} & \multicolumn{3}{c}{Nature + Human + Supernatural} \\"' ///
			`"\cmidrule(lr){2-4} \cmidrule(lr){5-7} \cmidrule(lr){8-10}"' ///
			`" & (1) & (2) & (3) & (4) & (5) & (6) & (7) & (8) & (9) \\"' ///
			`"\midrule"') ///
	postfoot(`" & & & & & & & & & \\"' ///
			 `" HII control & No & Yes & Yes & No & Yes & Yes & No & Yes & Yes \\"' ///
			 `" Climatic zones control & No & No & Yes & No & No & Yes & No & No & Yes \\"' ///
			 `" Country fixed effects &  Yes & Yes & Yes & Yes & Yes & Yes & Yes & Yes & Yes \\"' ///			 
			 `" & & & & & & & & & \\"' ///
			 `"Observations & ${n0} & ${n1} & ${n2} & ${n3} & ${n4} & ${n5} & ${n6} & ${n7} & ${n8} \\"' ///
			 `"Mean of dep. var. & ${meany0} & ${meany1} & ${meany2} & ${meany3} & ${meany4} & ${meany5} & ${meany6} & ${meany7} & ${meany8} \\"' ///
			 `"Standard deviation of dep. var. & ${sd0} & ${sd1} & ${sd2} & ${sd3} & ${sd4} & ${sd5} & ${sd6} & ${sd7} & ${sd8} \\"' ///			 
			 `"Ethnic-folklore clusters & ${cl0} & ${cl1} & ${cl2} & ${cl3} & ${cl4} & ${cl5} & ${cl6} & ${cl7} & ${cl8} \\"' ///
			 `"\bottomrule"' ///
			 `"\end{tabular}"')
